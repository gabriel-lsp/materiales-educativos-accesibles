(() => {
  ...
  let nivelClave='inicial', configuracion=niveles.inicial, universoActual=alfabeto.slice(0,10), ejercicios=[], indice=0, seleccion='', intentos=0, aciertosPrimerIntento=0, errores=0, pistasUsadas=0, modo='lectura', tipo='signo-letra', numerosVisibles=true;
  ...
  function renderizarOpciones(){
    ...
    if(tipo==='signo-letra'){
      letra.hidden=true;letras.hidden=false;signo.hidden=false;signos.hidden=true;signo.appendChild(crearSigno(objetivo));
      opcionesPara(objetivo).forEach(op=>{const b=document.createElement('button');b.type='button';b.className='opcion-letra';b.textContent=textoOpcion(op);b.setAttribute('aria-pressed','false');b.onclick=()=>seleccionar(letras,b,op.id);letras.appendChild(b);});
    }else{
      letra.hidden=false;letra.textContent=textoOpcion(objetivo);letras.hidden=true;signo.hidden=true;signos.hidden=false;
      opcionesPara(objetivo).forEach(op=>{const b=document.createElement('button');b.type='button';b.className='opcion-signo';b.setAttribute('aria-pressed','false');b.setAttribute('aria-label',`Opción Braille para ${op.nombre}`);b.appendChild(crearSigno(op,true));b.onclick=()=>seleccionar(signos,b,op.id);signos.appendChild(b);});
    }
  }
  ...
  tarjetas.forEach(t=>t.addEventListener('click',()=>{
    ...
    tarjetas.forEach(x=>{const activa=x===t;x.classList.toggle('activa',activa);x.setAttribute('aria-pressed',String(activa));});
  ...
  $('seleccionar-todos').addEventListener('click',()=>{casillasContenido.forEach(c=>{if(!c.disabled)c.checked=true;});actualizarResumenContenidos();});
  $('quitar-adicionales').addEventListener('click',()=>{casillasContenido.forEach(c=>{if(!c.disabled)c.checked=false;});actualizarResumenContenidos();});
  ...
  $('boton-comprobar').addEventListener('click',()=>{
    ...
    intentos+=1;const correcto=seleccion===ejercicios[indice].id;
  ...
  $('boton-pista').addEventListener('click',()=>{
    if(configuracion.pistas<=pistasUsadas)return;
    pistasUsadas+=1;
    ...
    resultado.className='resultado';resultado.textContent=mensaje;actualizarApoyos();
    if('speechSynthesis'in window){window.speechSynthesis.cancel();const voz=new SpeechSynthesisUtterance(mensaje);voz.lang='es-PE';window.speechSynthesis.speak(voz);}
  });
